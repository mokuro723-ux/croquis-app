    (function(){
        ...
        function skGestureEnd(e){
            ...
            if (n >= 2 && !gMoved && dur < 900) {
                if (n === 2) { skUndo(); skShowMsg('↩ 元に戻す（' + n + '本）'); }
                else { skRedo(); skShowMsg('↪ やり直し（' + n + '本）'); }
                setTimeout(function(){ skShowMsg(''); }, 1100);
            } else if (n >= 2) {
                // 判定外（動いた/長すぎ）でも、何本検出したかは表示して切り分けに使う
                skShowMsg('検出: ' + n + '本（タップとして認識されず）');
                setTimeout(function(){ skShowMsg(''); }, 1100);
            }
        }
